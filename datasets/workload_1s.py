...
5. Save raw data segments to HDF5 (Sliced to 1s)
"""
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for set_file in set_files:
                ...
                try:
                    ...
                    # Add segments (Modified for 1s slicing)
                    segment_counter = 0
                    window_size = int(self.target_sfreq * 1.0) # 1s window

                    for i in range(data.shape[0]):
                        epoch_data = data[i] # (n_channels, n_times)
                        
                        # Reorder/Select channels
                        mapped_data = np.zeros((len(valid_channels), epoch_data.shape[1]), dtype=np.float32)
                        for target_idx, src_idx in enumerate(file_ch_map):
                            if src_idx != -1:
                                mapped_data[target_idx] = epoch_data[src_idx]
                        
                        # Slice into 1s segments
                        total_samples = mapped_data.shape[1]
                        num_segments = total_samples // window_size
                        
                        if num_segments == 0:
                            print(f"  Warning: Data length {total_samples} < window size {window_size}. Skipping.")
                            continue

                        for seg_idx in range(num_segments):
                            start_idx = seg_idx * window_size
                            end_idx = start_idx + window_size
                            
                            seg_data = mapped_data[:, start_idx:end_idx]
                            
                            seg_attrs = SegmentAttrs(
                                segment_id=segment_counter,
                                start_time=seg_idx * 1.0, 
                                end_time=(seg_idx + 1) * 1.0,
                                time_length=1.0,
