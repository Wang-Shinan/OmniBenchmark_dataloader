...
- Raw data including 25 channels with last three EOG
...
class BCIC2ABuilder:
    ...
    def _read_raw_mat(self, file_path: Path):
        ...
        for run_idx in range(len(data_struct[0]) - 6, len(data_struct[0])):  ## remove those EOG sessions 
            ...
            if trial_indices.size > 0 and y.size > 0:
                ...
                trial_times = (cumulative_samples + trial_indices_flat) / sfreq + 2.0 # so as to remove the fixation and include MI carry out time
    ...
    def build_subject(self, subject_id: int) -> dict:
        """
    ...
    def build_all(self, subject_ids: Optional[list[int]] = None, max_workers: int = 4) -> list[str]:
        """
        Build HDF5 files for all subjects in parallel.
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            # Submit all tasks
            future_to_subject = {
                executor.submit(self.build_subject, subject_id): subject_id
            ...
            for future in as_completed(future_to_subject):
                ...
                try:
                    ...
                    all_total_trials += result['total_trials']
                    all_valid_trials += result['valid_trials']
                    all_rejected_trials += result['rejected_trials']
                    all_valid_segments += result['valid_segments']
...
def build_bcic2a(
    ...
    **kwargs,
    ...
    return builder.build_all(subject_ids, max_workers=max_workers)
...
if __name__ == "__main__":
    ...
    build_bcic2a(args.raw_data_dir, args.output_dir, args.subjects, max_workers=args.workers, file_format=args.format)
