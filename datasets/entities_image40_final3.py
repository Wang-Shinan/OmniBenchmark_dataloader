"""EntitiesEEG - Image modality (40-class concept) builder (final)
...
- Blocks: "Image_1_trial_start" ... "Image_8_trial_end"

Segmentation:
- duration_log_image_group<block>.csv  (block = 0..7)
...
Quick debug (1 subject, 1 block, compare with legacy labels):
  python entities_image40_final.py "Z:\Datasets\Entities" --output_dir "Z:\Processed_datasets\EEG_Bench" --subjects sub3 --debug_block 0 --compare_labels "Z:\Datasets\Entities\Datasets\image_labels.npy"
"""
...
class EntitiesImage40BuilderFinal:
    def __init__(
        self,
        raw_root: str,
        output_dir: str,
        ...
        self.out_dir = Path(output_dir) / "EntitiesImage40"
    ...
    def build_subject(self, sub_dir: Path, debug_block: Optional[int] = None, compare_labels: Optional[str] = None) -> Optional[str]:
        ...
        blocks = _extract_blocks(events, event_id, sfreq0, "Image")
        ...
        with HDF5Writer(str(out_path), subject_attrs) as writer:
            for blk in blocks:
                ...
                df = _read_duration_log(self.duration_log_dir, "image", bi)
                stim_events = _expand_stimulus_events(df, "image")
                ...
                trial_name = writer.add_trial(TrialAttrs(trial_id=bi, session_id=0, task_name="image"))
    ...
    def build_all(
        ...
        note = (
            "Image modality only; triggers via template code_to_name + events_from_annotations; "
...
def main():
    ...
    builder = EntitiesImage40BuilderFinal(
    ...
    print(f"[EntitiesImage40] Done. H5 files: {len(outs)} -> {builder.out_dir}")
