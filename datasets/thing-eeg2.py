...
YOUR_DATASET_INFO = DatasetInfo(
    ...
    num_labels=16540,                 #change with file
    category_list=["category.csv"],   
    sampling_rate=100.0,
    montage="10_20",
...
class YourDatasetBuilder:
    ...
    def __init__(
        ...
        self.output_dir = Path(output_dir) / "Thing_EEG2_training"  #change with file 
    ...
    def _find_files(self, subject_id: int):
        ...
        files = [
            sub_dir / 'preprocessed_eeg_training.npy',                # use one path at a time
            ...
            *lead, n_ch, n_tp = data.shape
            ...
            for trial_id in range(n_total):
                all_trials.append({
                    ...
                    "label": self._get_label(trial_id % 16540),  # change with data
                    "trial_id": trial_id,
                    "session_id": trial_id // 16540,  # change with data
                    "onset_time": 0.0,
                })
            print(1)
        return all_trials      
    ...
    def _get_label(self,trial_id):
        ...
        key='train_img_concepts'           #change with file
        ...
        label = int(concept.split('_')[0].lstrip('0') or '0')
        return label
