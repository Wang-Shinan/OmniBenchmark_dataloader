...
class CCBDRestBuilder:
    ...
    def get_subject_ids(self) -> list[str]:
        ...
        # 1. raw_data_dir/sub-xxxxxxx/ses-visit1/*.set (with session directory)
        # 2. raw_data_dir/sub-xxxxxxx/*.set (without session directory)
        for item in self.raw_data_dir.iterdir():
            if item.is_dir() and item.name.startswith("sub-"):
                ...
                found_set_file = False
                
                # First, check if there are ses-* subdirectories with .set files
                ...
                # If no session directory found, check directly in sub-* directory
                if not found_set_file:
    ...
    def _find_files(self, subject_id: str) -> Path:
        ...
        1. raw_data_dir/sub-{subject_id}/ses-visit1/*.set (with session directory)
        2. raw_data_dir/sub-{subject_id}/*.set (without session directory)
        """
        subject_dir = self.raw_data_dir / f"sub-{subject_id}"
        if not subject_dir.exists():
            raise FileNotFoundError(f"Subject directory not found: {subject_dir}")
        
        # First, try to find .set file in ses-* subdirectories
        ...
        # If no session directory found, look directly in sub-* directory
        # Try pattern sub_{subject_id}.set first (new naming)
        set_file = subject_dir / f"sub_{subject_id}.set"
        if set_file.exists():
            return set_file
        
        # Try old pattern {subject_id}_Resting-prepro.set
        set_file = subject_dir / f"{subject_id}_Resting-prepro.set"
        if set_file.exists():
            return set_file
        
        # Try any .set file in the subject directory
